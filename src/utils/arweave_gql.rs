...
use crate::utils::{constants::ARWEAVE_GQL_GATEWAY, planetscale::ps_get_latest_block_id};
use anyhow::Error;
...
async fn retrieve_all_transactions(scan_count: u32, address: &str) -> Result<Vec<u32>, Error> {
    ...
    loop {
        ...
        let query = json!({
            "query": r#"
            query GetTransactions($cursor: String, $pageSize: Int!, $owner: String!) {
                transactions(
                    first: $pageSize,
                    after: $cursor,
                    sort: HEIGHT_ASC,
...
pub async fn detect_missing_blocks(scan_count: u32) -> Result<Vec<u32>, Error> {
    ...
    let latest_archived_block = ps_get_latest_block_id().await as u32;
    ...
    let mut canonical_chain_blocks: Vec<u32> = (latest_archived_block..=latest_block).collect();
